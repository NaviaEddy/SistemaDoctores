...
<body>
                                ...
                                <tbody>
                                    ...
                                    <br><br><br><br>
                                        ...
                                        for ($x = 0; $x < $result->num_rows; $x++) {
                                            ...
                                            if ($isActive == 1) {
                                                echo '<a href="?action=deactivate&id=' . $pid . '" class="non-style-link">
                                                                    <button class="btn-warning btn button-icon btn-deactivate" 
                                                                            style="padding: 12px 20px; margin-top: 10px; display: inline-flex; align-items: center; justify-content: center;">
                                                                        <span class="tn-in-text">Disable</span>
                                                                    </button>
                                                        </a>';
                                            } else {
                                                echo '<a href="?action=activate&id=' . $pid . '" class="non-style-link">
                                                                    <button class="btn-primary btn button-icon btn-activate" 
                                                                            style="padding: 12px 20px; margin-top: 10px; display: inline-flex; align-items: center; justify-content: center;">
                                                                        <span class="tn-in-text">Enable</span>
                                                                    </button>
                                                        </a>';
                                            }
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'deactivate' && isset($_GET['id'])) {
        $pid = $_GET['id'];
        $query = "UPDATE patient SET isActive=0 WHERE pid='$pid'";
        if ($database->query($query) === TRUE) {
            echo "<script>window.location.href = 'patient.php';</script>";
        } else {
            echo "Error al desactivar la cuenta: " . $database->error;
        }
    }

    if (isset($_GET['action']) && $_GET['action'] == 'activate' && isset($_GET['id'])) {
        $pid = $_GET['id'];
        $query = "UPDATE patient SET isActive=1 WHERE pid='$pid'";
        if ($database->query($query) === TRUE) {
            echo "<script>window.location.href = 'patient.php';</script>";
        } else {
            echo "Error al activar la cuenta: " . $database->error;
        }
    }
